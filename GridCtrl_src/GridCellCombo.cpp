						   ...
						   UINT nFirstChar)
{
    ...
    // If you want you can override the array of strings here, based on the row and column
    // that this combo will be displayed at. For instance for this demonstration only, we'll
    // plug in some pre-defined strings. Use CGridCellCombo::SetOptions to set the strings
    // at runtime.
    /*
    Items.RemoveAll();
    Items.Add(_T("Option 1"));
    Items.Add(_T("Option 2"));
    Items.Add(_T("Option 3"));
    Items.Add(_T("Option 4"));
    Items.Add(_T("A really long Option 5"));
    */
    m_crForeClr = crFore;
    m_crBackClr = crBack;
...
void CInPlaceList::OnEditUpdate() 
{
...
CSize CGridCellCombo::GetTextExtent(LPCTSTR str, CDC* pDC)
{
	//return CGridCell::GetTextExtent(str, pDC);
	CSize cSize = CGridCell::GetTextExtent(str, pDC);
	cSize += pDC->GetTextExtent("    ", 4);
	if (cSize.cx > 96)
		cSize.cx = 96;	 //Piers hack so that everything fits even with long name
	return cSize;
}
